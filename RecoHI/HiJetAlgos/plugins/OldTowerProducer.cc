// -*- C++ -*-
//
// Package:    OldTowerProducer
// Class:      OldTowerProducer
// 
/**\class OldTowerProducer OldTowerProducer.cc RecoHI/OldTowerProducer/src/OldTowerProducer.cc
...
#include "RecoHI/HiJetAlgos/plugins/OldTowerProducer.h"
...
//
OldTowerProducer::OldTowerProducer(const edm::ParameterSet& iConfig):
   geo_(0)
{
...
OldTowerProducer::~OldTowerProducer()
{
...
void
OldTowerProducer::produce(edm::Event& iEvent, const edm::EventSetup& iSetup)
{
   using namespace edm;
    ...
    */
    ...
    //    cout<<"accumulated et :"<<towers_[hid]<<endl;
   ...
   for ( std::map< DetId, double >::const_iterator iter = towers_.begin();
     ...
     //     cout<<"Creatin tower et : "<<et<<" | ieta : "<<newTowerId.ieta()<<" | iphi : "<<newTowerId.iphi()<<endl;

     //     cout<<"at position | eta : "<<pos.eta()<<" | phi : "<<pos.phi()<<endl;
...
void 
OldTowerProducer::endJob() {
}


void OldTowerProducer::resetTowers(edm::Event& iEvent,const edm::EventSetup& iSetup)
{
...
DetId OldTowerProducer::getNearestTower(const reco::PFCandidate & in) const {
...
DetId OldTowerProducer::getNearestTower(double eta, double phi) const {
...
int OldTowerProducer::eta2ieta(double eta) const {
  ...
  for (iter = 0; iter < size ; iter++) {
    ...
    if( curr >= size || curr < 1 )
      std::cout <<  " OldTowerProducer::eta2ieta - wrong current index = "
		<< curr << " !!!" << std::endl;
...
int OldTowerProducer::phi2iphi(double phi, int ieta) const {
...
DEFINE_FWK_MODULE(OldTowerProducer);
