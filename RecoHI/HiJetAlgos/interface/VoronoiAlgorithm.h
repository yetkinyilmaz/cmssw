...
#define BPMPD_VERSION_STRING "Version 2.11 (1996 December)"
	...
	public:
		...
		int optimize(void)
		{
			...
			dims_.nz = nonzero_value.size();

			dims_.cfree = 2 * (dims_.nz + 1);
			dims_.rfree = dims_.nz;
			...
			double addobj = 0;
			double bigbou = 1e+15;
			double infinity_copy = infinity;
			int ft;

			_variable_primal.resize(dims_.mn);
			_variable_dual.resize(dims_.m);

			solver_(&_objective[0], &_rhs[0], &_lower_bound[0],
					&_upper_bound[0], &diag[0], &odiag[0],
					&_variable_primal[0], &dxs[0], &dxsn[0], &up[0],
					&dual_residual[0], &ddspr[0], &ddsprn[0], &dsup[0],
					&ddsup[0], &ddsupn[0], &_variable_dual[0], &ddv[0],
					&ddvn[0], &prinf[0], &upinf[0], &duinf[0], &scale[0],
...
namespace {
	...
	protected:
		...
		bool _radial_distance_square_max;
		bool _subtracted;
		event_t _event;
		...
		float ue_interpolation_pf0[7][344];
		float ue_interpolation_pf1[7][344];
		float ue_interpolation_pf2[7][82];
#endif // STANDALONE
	private:
		void initialize_geometry(void)
		{
			...
			for(int i = 0; i < 3; ++i){
			   for(int j = 0; j < 7; ++j){
				 ...
				 for(int l = 0; l < 2; ++l){
				    for(int m = 0; m < 46; ++m){				       
				       ue_predictor_pf[i][j][k][l][m] = ue::ue_predictor_pf[i][j][k][l][m];
				    }
				 }
			      }
			   }
                        ...
                        for(int i = 0; i < 7; ++i){
			   for(int j = 0; j < 344; ++j){
			      ue_interpolation_pf0[i][j] = ue::ue_interpolation_pf0[i][j];
			      ue_interpolation_pf1[i][j] = ue::ue_interpolation_pf1[i][j];
			   }
			}

			for(int i = 0; i < 7; ++i){
                           for(int j = 0; j < 82; ++j){
			      ue_interpolation_pf2[i][j] = ue::ue_interpolation_pf2[i][j];
			   }
			}
#endif // STANDALONE
		}
		...
		void subtract_momentum(void)
		{
			for (std::vector<particle_t>::iterator iterator =
				...
				if (_remove_nonpositive) {
					// Oversubtraction fits for each of the eta ranges
					// for R = 0.3, needs to become a external data
...
p4 = 1.13735251e+00;
					...
					for (size_t l = 0; l < 10; l++) {
						float y = x - std::max(0.0, p0 + p1 * x - p2 * exp(-p3 * pow(x, p4)));
						x += pred_0 - y;
					}
					...
					if (std::isfinite(iterator->area)) {
						...
						iterator->momentum_perp_subtracted =
							iterator->momentum.perp() -
#if 0
							// FIXME: Temporary fix for validation.
							// Oversubtraction currently not working
							// for pp.
							(x / pred_0) *
#endif
		...
		void lp_populate(bpmpd_problem_t &_lp_problem)
		{
			...
			_lp_problem.set_objective_sense(
				bpmpd_problem_t::minimize);
			...
			for (; iterator_particle != _event.end();
				...
				if (*iterator_active) {
					...
					_lp_problem.push_back_coefficient(
						index_row, 0, iterator_particle->area);

					// Energy transfer coefficients t_kl
					const double sign = iterator_particle->
						momentum_perp_subtracted >= 0 ? -1 : 1;
					...
					for (std::vector<size_t>::const_iterator
						...
						const size_t index_column =
							*iterator_recombine_index_inner + 1;
			...
			// Epsilon that breaks the degeneracy, in the same units as
			// the pT of the event (i.e. GeV)
			static const double epsilon_degeneracy = 1e-2;

			// Columns (variables and the objective coefficients) of
			// the LP problem
			//
			// Cost variable (objective 1)
			_lp_problem.push_back_column(
				1, 0, bpmpd_problem_t::infinity);
			// Energy transfer coefficients t_kl (objective 0 + epsilon)
			for (size_t i = 1; i <= index_column_max; i++) {
				_lp_problem.push_back_column(
					epsilon_degeneracy * _recombine_tie[i - 1],
		...
		void equalize(void)
		{
			...
			for (size_t k = 1; k < x.size(); k++) {
				_event[_recombine[k - 1].first].
					momentum_perp_subtracted += x[k];
				_event[_recombine[k - 1].second].
					momentum_perp_subtracted -= x[k];
			}
		}
